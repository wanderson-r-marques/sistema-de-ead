...
$(function () {
  ...
  $('.candidato').click(function () {
    ...
    // Resetando as PKs no CONFIRMAR e ELIMINAR
    $('#confirmar').attr('pk', '');
    $('#elimminar').attr('pk', '');
    $("input[name='valor_pag']").val('');
    $("input[name='data_pag']").val('');
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $('#m_inscricao').html('<b>Inscrição:</b> ' + response[0].inscricao);
        $('#m_candidato').html('<b>Nome:</b> ' + response[0].nome);
        $('#m_cpf').html('<b>CPF:</b> ' + response[0].cpf);
        $('#m_modalidade').html('<b>Modalidade:</b> ' + response[0].modalidade);

        // Adicionando arquivo
        if (response[0].doc_deposito) {
          $('.doc_deposito').removeClass('d-none');
          $('.v_doc_deposito').attr('href', response[0].doc_deposito);
          $('.e_doc_deposito').attr('pk', response[0].inscricao);
        } else {
          $('.doc_deposito').addClass('d-none');
          $('.v_doc_deposito').attr('href', '#');
          $('.e_doc_deposito').attr('pk', '');
        }
        // Adicionando arquivo
        if (response[0].doc_rg) {
          $('.doc_rg').removeClass('d-none');
          $('.v_doc_rg').attr('href', response[0].doc_rg);
          $('.e_doc_rg').attr('pk', response[0].inscricao);
        } else {
          $('.doc_rg').addClass('d-none');
          $('.v_doc_rg').attr('href', '#');
          $('.e_doc_rg').attr('pk', '');
        }
        // Adicionando arquivo
        if (response[0].doc_cpf) {
          $('.doc_cpf').removeClass('d-none');
          $('.v_doc_cpf').attr('href', response[0].doc_cpf);
          $('.e_doc_cpf').attr('pk', response[0].inscricao);
        } else {
          $('.doc_cpf').addClass('d-none');
          $('.v_doc_cpf').attr('href', '#');
          $('.e_doc_cpf').attr('pk', '');
        }
        // Adicionando arquivo
        if (response[0].doc_residencia) {
          $('.doc_residencia').removeClass('d-none');
          $('.v_doc_residencia').attr('href', response[0].doc_residencia);
          $('.e_doc_residencia').attr('pk', response[0].inscricao);
        } else {
          $('.doc_residencia').addClass('d-none');
          $('.v_doc_residencia').attr('href', '#');
          $('.e_doc_residencia').attr('pk', '');
        }
        // Adicionando arquivo
        if (response[0].doc_diploma) {
          $('.doc_diploma').removeClass('d-none');
          $('.v_doc_diploma').attr('href', response[0].doc_diploma);
          $('.e_doc_diploma').attr('pk', response[0].inscricao);

        } else {
          $('.doc_diploma').addClass('d-none');
          $('.v_doc_diploma').attr('href', '#');
          $('.e_doc_diploma').attr('pk', '');
        }
        // Adicionando arquivo
        if (response[0].doc_carteira) {
          $('.doc_carteira').removeClass('d-none');
          $('.v_doc_carteira').attr('href', response[0].doc_carteira);
          $('.e_doc_carteira').attr('pk', response[0].inscricao);

        } else {
          $('.doc_carteira').addClass('d-none');
          $('.v_doc_carteira').attr('href', '#');
          $('.e_doc_carteira').attr('pk', '');
        }
        // Adicionando arquivo
        if (response[0].doc_certidao) {
          $('.doc_certidao').removeClass('d-none');
          $('.v_doc_certidao').attr('href', response[0].doc_certidao);
          $('.e_doc_certidao').attr('pk', response[0].inscricao);

        } else {
          $('.doc_certidao').addClass('d-none');
          $('.v_doc_certidao').attr('href', '#');
          $('.e_doc_certidao').attr('pk', '');
        }


        // Colocando as PKs no CONFIRMAR e ANULAR
        $('#confirmar').attr('pk', response[0].inscricao);
        $('#anular').attr('pk', response[0].inscricao);
  ...
  // Atualizar isento
  $("input[name='isento']").click(function () {
    var pk = $(this).attr('pk');
    var funcao = 'isencao';
  ...
  // Deletar arquivo
  $('.excluir_doc').click(function () {
    if (confirm('Deseja deletar esse documento?')) {
      var pk = $(this).attr('pk');
      var tabela = $(this).attr('table');
      var funcao = 'deletar_doc';
      var arquivo = $(this).prev().prev().attr('href');
  ...
  // Confirmmar inscrição
  $('#confirmar').click(function () {

    var data_pag = $("input[name='data_pag']").val();
    var valor_pag = $("input[name='valor_pag']").val();

    if (data_pag != '' && valor_pag != '' && parseInt(data_pag.length) == 10) {

      var funcao = 'confirmar';
      var pk = $(this).attr('pk');
      ...
      $.ajax({
        ...
        success: function (response) {
          console.log(response);
          if (response == 'true') {
            $('a[pk=' + pk + ']').removeClass('text-warning');
            $('a[pk=' + pk + ']').removeClass('text-danger');
            $('a[pk=' + pk + ']').addClass('text-success');
            $("#exampleModal").modal('hide');
          }
        }
      });
    } else {
      alert('É preciso preencher o campo de VALOR e DATA do pagamento!');
    }
  });
  ...
  $('#anular').click(function () {
    ...
    if (pk != '' && pk > 0 && confirm('Deseja anular essa inscrição?')) {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response == 'true') {
            $('a[pk=' + pk + ']').removeClass('text-warning');
            $('a[pk=' + pk + ']').removeClass('text-success');
            $('a[pk=' + pk + ']').addClass('text-danger');
            $("#exampleModal").modal('hide');
          }
        }
      });
...
(function (f) { "function" === typeof define && define.amd ? define(["jquery"], function (z) { return f(z, window, document) }) : "object" === typeof exports ? module.exports = function (z, y) { z || (z = window); y || (y = "undefined" !== typeof window ? require("jquery") : require("jquery")(z)); return f(y, z, z.document) } : f(jQuery, window, document) })(function (f, z, y, p) {
  ...
  q.version = "1.10.20"; q.settings = []; q.models = {}; q.models.oSearch = { bCaseInsensitive: !0, sSearch: "", bRegex: !1, bSmart: !0 }; q.models.oRow = { nTr: null, anCells: null, _aData: [], _aSortData: null, _aFilterData: null, _sFilterRow: null, _sRowStripe: "", src: null, idx: -1 }; q.models.oColumn = {
    ...
    fnStateLoadParams: null, fnStateLoaded: null, fnStateSaveCallback: function (a, b) { try { (-1 === a.iStateDuration ? sessionStorage : localStorage).setItem("DataTables_" + a.sInstance + "_" + location.pathname, JSON.stringify(b)) } catch (c) { } }, fnStateSaveParams: null, iStateDuration: 7200, iDeferLoading: null, iDisplayLength: 10, iDisplayStart: 0, iTabIndex: 0, oClasses: {}, oLanguage: {
      ...
      }, sEmptyTable: "Não há registros!", sInfo: "Exibindo _START_ até _END_ de _TOTAL_ registros", sInfoEmpty: "Exibindo 0 até 0 de 0 registros", sInfoFiltered: "(filtrado de _MAX_ registros totais)", sInfoPostFix: "", sDecimal: "", sThousands: ",", sLengthMenu: "Exibindo _MENU_ registros", sLoadingRecords: "Carregando...", sProcessing: "Processando...", sSearch: "Procurar:", sSearchPlaceholder: "", sUrl: "", sZeroRecords: "No matching records found"
