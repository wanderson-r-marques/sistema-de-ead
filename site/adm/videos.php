...
<td><img src="../uploads/adm_<?= $reg['imagem']; ?>"  border="0" class="img-polaroid"> </td>
   <td>
    ...
    <td>
		<a rel="tooltip" href="?pag=videos&op=edit&id=<?= $reg['id'];?>" class="btn" title="Editar" ><i class="icon-pencil"></i></a>
		<a rel="tooltip" href="?pag=videos&op=sta&id=<?= $reg['id'];?>&status=<?= $status;?>" class="btn" title="<?= $title; ?>" ><i class="icon-eye-<?= $icon; ?>"></i></a>
		<a rel="tooltip" href="?pag=videos&op=exc&id=<?= $reg['id'];?>" class="btn" title="Excluir" ><i class="icon-trash"></i></a>	</td>
  </tr>
	<?php } ?>
...
<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=videos&op=add_now" >
  ...
  <div  class="control-group" >
    ...
    <div class="controls" >
	   <input name="url" value="" type="text" required="required" class="input-xxlarge" id="url" placeholder="Adicione a url do vídeo">		
    </div>
  </div>
...
<!--   <div  class="control-group" >
    ...
    <div class="controls" >
	   <input name="categoria" value="" type="text" required="required" class="input-xxlarge" id="categoria" placeholder="Adicione a categoria do vídeo">		
    </div>
  </div> -->
  ...
  <div   class="control-group ">
    ...
    <div class="controls" >
	      <input name="titulo" value="" type="text" required="required" class="input-xxlarge" id="titulo" placeholder="Adicione um título ">		  
    </div>
  </div> 
...
<!-- 
  <div   class="control-group ">
    ...
    <div class="controls" >
	      <input name="descricao" value="" type="text" required="required" class="input-xxlarge" id="descricao" placeholder="Adicione uma descrição ">		  
    </div>
  </div> 
     -->
      <div  class="control-group">
    <label class="control-label"  for="fileInput">Foto de capa</label>
    <div class="controls">
...
<? if($op == "add_now"){ 
	...
	//testando campos da noticia
		if( $titulo == "" ||  $imagem == ""|| $url == "" ){  echo "<meta http-equiv='refresh' content='0;URL=?pag=videos&op=add&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>";  die(); }
	...
	$sql= mysql_query("INSERT INTO $pagina (url,titulo,descricao,categoria,imagem,  data, hora, status )values('$url','$titulo','$descricao','$categoria','$imagem','$data','$hora','sim')") ;
...
<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=videos&op=edit_now&id=<?= $id; ?>" >
...
<!--   <div   class="control-group ">
    <label class="control-label" for="fileInput">Categoria do Vídeo</label>
    <div class="controls" >
	      <input name="categoria" value="<?php echo aspas($reg['categoria']); ?>" type="text" class="input-xxlarge" id="categoria" maxlength="90" placeholder="Adicione uma categoria ">
    </div>
  </div>  -->
  ...
  <div   class="control-group ">
    <label class="control-label" for="fileInput">Título do Vídeo</label>
    <div class="controls" >
	      <input name="titulo" value="<?php echo aspas($reg['titulo']); ?>" type="text" class="input-xxlarge" id="titulo" maxlength="90" placeholder="Adicione um título ">
    </div>
  </div> 
...
<!--     <div   class="control-group ">
    <label class="control-label" for="fileInput">Descrição do Vídeo</label>
    <div class="controls" >
	      <input name="descricao" value="<?php echo aspas($reg['descricao']); ?>" type="text" class="input-xxlarge" id="descricao" maxlength="90" placeholder="Adicione uma descrição ">
    </div>
  </div>  -->
    <div class="control-group">
...
<? if($op == "edit_now"){ 
	...
	if($titulo == "" || $url == "" ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=videos&op=edit&id=$id&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>";  die(); }	
	...
	$sql= mysql_query("UPDATE $pagina SET url='$url',titulo='$titulo',descricao='$descricao',categoria='$categoria',imagem='$imagem',hora='$hora',data='$data' WHERE id = '$id' ") ;
...
<? if($op == "exc"){ 			
	...
	if($msg != "exc"){ ?>
		...
		if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=videos&msg=erro&txt=Ocorreu um erro ao excluir o evento'>"; }
