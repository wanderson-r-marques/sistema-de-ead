<?php require_once("includes/verifica.php"); $pagina = "inscricao";
...
<ul class="breadcrumb bord" >
  ...
  <li><a href="?pag=inscricao">Inscrição</a> <span class="divider">/</span></li>
</ul>
...
$(document).ready(function(){
    $(".btn").click(function(){
        ...
        $("#show").load("inscricaoView.php?id="+id);
    });
});
</script>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered table-hover">
  <thead>
    <tr>
      ...
      <th width="50">A&ccedil;&atilde;o</th>
    </tr>
  </thead>
  ...
  <tr>
    ...
    <td><?= $reg['nome']; ?></td>
    <td><a rel="tooltip" data-id="<? echo $reg['id'];?>" data-toggle="modal" class="btn" title="Ver detalhes" ><i class="icon-list-alt"></i></a>  </td>
  </tr>
  <?php } ?>
</table>
<?
...
<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=inscricao&op=add_now">
...
<? if($op == "add_now"){ 
	...
	//testando campos da noticia
		if( $titulo == "" || $arquivo == "" ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&op=add&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>"; die(); }
	...
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&op=add&msg=erro&txt=Ocorreu um erro ao inserir'>"; }
	
		//else if($imagem == ""){  echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&msg=aviso&txt=<b>Imagem não enviada!</b>  inserida com sucesso'>"; }
			
			else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&msg=sucesso&txt= inserido com sucesso'>"; }
...
<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=inscricao&op=edit_now&id=<?= $id; ?>" >
 <div class="control-group" >
    <label class="control-label" for="fileInput">Data e hora</label>
    <div class="controls controls-row">
      <input name="data" type="text" readonly="" class="input-small" id="data" value="<?= $data; ?>" >
...
<? if($op == "edit_now"){ 
	...
	//testando campos da noticia
		if( $titulo == ""  ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&op=edit&id=$id&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>";  die(); }	
	...
	//testando campos da noticia
		if( $titulo == "" ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&op=add&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>"; die(); }
	...
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&op=edit&id=$id&msg=erro&txt=Ocorreu um erro ao editar'>"; }
	
		//else if($imagem == ""){  echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&msg=aviso&txt=<b>Imagem não enviada!</b> Download inserida com sucesso'>"; }
			
			else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&msg=sucesso&txt=Item editada com sucesso'>"; }
...
<div class="alert alert-block alert-error fade in"> <a href="#" class="close" data-dismiss="alert">×</a>
  ...
  <p> <a class="btn btn-danger" href="?pag=inscricao&op=exc&id=<?= $id;?>&msg=exc">Sim</a> <a class="btn" href="?pag=inscricao">Não</a> </p>
</div>
<? }
	
	else{ $sql = mysql_query("DELETE FROM $pagina where id='$id' LIMIT 1"); 
	
		if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&msg=erro&txt=Ocorreu um erro ao excluir a notícia'>"; }
	
		else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=inscricao&msg=sucesso&txt=Item excluído com sucesso'>"; }
