<?php require_once("includes/verifica.php"); $pagina = "post";
...
<a href="?pag=<?php echo $pagina; ?>&op=add" class="btn btn-primary" ><i class="icon-plus icon-white"></i> Adicionar novo post </a><br />
<br />
<?php
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered table-hover">
  ...
  <tr>
    <td><?php echo $data; ?> </td>
     <td><?php echo aspas($reg2['nome']); ?></td>
    <td><?php echo aspas($reg['titulo']); ?></td>
    <td><a href="?pag=post&op=edit&id=<?php echo $reg['id'];?>" class="btn" title="Editar" ><i class="icon-pencil"></i></a> <a href="?pag=post&op=sta&id=<?php echo $reg['id'];?>&status=<?php echo $status;?>" class="btn" title="<?php echo $title; ?>" ><i class="icon-eye-<?php echo $icon; ?>"></i></a> <a href="?pag=post&op=exc&id=<?php echo $reg['id'];?>" class="btn" title="Excluir" ><i class="icon-trash"></i></a> </td>
  </tr>
  <?php } ?>
</table>
<?
...
<ul class="breadcrumb bord" >
  <li><a href="?">Home</a> <span class="divider">/</span></li>
  <li><a href="?pag=post">Colunas Socias</a> <span class="divider">/</span></li>
  <li>Adicionar Post</li>
</ul>
<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=post&op=add_now">
  ...
  <div class="control-group" id="tipo" >
    <label class="control-label" for="fileInput">Selecione seu perfil</label>
    <div class="controls">
      <select name="perfil" id="perfil" class="input-large">  
      	<?php $sql2 = mysql_query("SELECT * FROM colunistas order by nome asc ");
				while($reg2 = mysql_fetch_array($sql2)){ ?>
        <option  value="<?php echo $reg2['id']; ?>"><?php echo $reg2['nome']; ?></option>
		<?php } ?> 
      </select> 
    </div>
  </div>
  ...
  <div   class="control-group ">
    ...
    <div class="controls" >
      <input name="titulo" value="" type="text" class="input-xxlarge" id="titulo" maxlength="50" placeholder="Adicione um titulo com 50 caracteres">
  ...
  <div class="control-group">
    <div class="controls">
      <button type="submit" class="btn btn-primary" onclick="return validaPost()"><i class="icon-ok icon-white"></i> Salvar</button>
	 </div>
  </div>
</form>
<? } ?>
<? if($op == "add_now"){ 
	...
	//testando campos da noticia
		if( $titulo == "" ||  $texto == "" ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=post&op=add&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>"; die(); }
	...
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=post&op=add&msg=erro&txt=Ocorreu um erro ao inserir o post'>"; }
				
			else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=post&msg=sucesso&txt=Post inserido com sucesso'>"; }
...
<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=post&op=edit_now&id=<?php echo $id; ?>" >
 <div class="control-group" >
    <label class="control-label" for="fileInput">Data e hora</label>
    <div class="controls controls-row">
      <input name="data" type="text" readonly="" class="input-small" id="data" value="<?php echo $data; ?>" >
  ...
  <div class="control-group" id="tipo" >
    ...
    <div class="controls">
      <select name="perfil" id="perfil" class="input-large">  
     	<?php $sql2 = mysql_query("SELECT * FROM colunistas where id = '$reg[perfil]'");
			$reg2 = mysql_fetch_array($sql2);?>
        <option  value="<?php echo $reg2['id']; ?>"><?php echo $reg2['nome']; ?></option>
      	<?php $sql2 = mysql_query("SELECT * FROM colunistas order by nome asc ");
				while($reg2 = mysql_fetch_array($sql2)){ ?>
  ...
  <div class="control-group ">
    <div class="controls">
      <button type="submit" class="btn btn-primary"  onclick="return validaEditPost()"><i class="icon-ok icon-white"></i> Salvar</button>
...
<? if($op == "edit_now"){ 
	...
	//testando campos da noticia
		if( $titulo == "" ||  $texto == ""  ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=post&op=edit&id=$id&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>";  die(); }	
	...
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=post&op=edit&id=$id&msg=erro&txt=Ocorreu um erro ao editar o post'>"; }
				
			else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=post&msg=sucesso&txt=Post editado com sucesso'>"; }
...
<ul class="breadcrumb bord" >
  <li><a href="?">Home</a> <span class="divider">/</span></li>
  <li><a href="?pag=post">Posts</a> <span class="divider">/</span></li>
  <li>Excluir Post</li>
</ul>
<div class="alert alert-block alert-error fade in"> <a href="#" class="close" data-dismiss="alert">×</a>
  <h4 class="alert-heading">Excluir Post</h4>
  <p>Tem certeza que deseja excluir esto post? Esta operação não pode ser desfeita.</p>
  <p> <a class="btn btn-danger" href="?pag=post&op=exc&id=<?php echo $id;?>&msg=exc">Sim</a> <a class="btn" href="?pag=post">Não</a> </p>
</div>
<? }
	
	else{ $sql = mysql_query("DELETE FROM $pagina where id='$id' LIMIT 1"); 
	
		if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=post&msg=erro&txt=Ocorreu um erro ao excluir o post'>"; }
	
		else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=post&msg=sucesso&txt=Post excluído com sucesso'>"; }
