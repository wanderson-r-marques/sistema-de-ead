...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered table-hover">
 <thead><tr>
  ...
  <tr>
    ...
    <td><?= $reg['titulo']; ?></td>
    <td>
		<a href="?pag=sessao_3&op=edit&id=<?= $reg['id'];?>" class="btn" title="Editar" ><i class="icon-pencil"></i></a>
		<a href="?pag=sessao_3&op=sta&id=<?= $reg['id'];?>&status=<?= $status;?>" class="btn" title="<?= $title; ?>" ><i class="icon-eye-<?= $icon; ?>"></i></a>
		<a href="?pag=sessao_3&op=exc&id=<?= $reg['id'];?>" class="btn" title="Excluir" ><i class="icon-trash"></i></a>	</td>
  </tr>
	<?php } ?>
...
<ul class="breadcrumb bord" >
  <li><a href="?">Home</a> <span class="divider">/</span></li>
  <li><a href="?pag=noticias">Notícias</a> <span class="divider">/</span></li>
  <li><a href="?pag=sessao_3">3º Bloco</a> <span class="divider">/</span></li>
  <li>Adicionar Notícia</li>
</ul>

<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=sessao_3&op=add_now" onclick="return validaNoticia2()">
...
<div class="control-group" >
  ...
  <div  class="control-group ">
    <label class="control-label" for="fileInput">Categoria da Notícia</label>
    <div class="controls" >
      <input name="categoria" value="<?= $reg['categoria']; ?>" type="text" class="input-large" id="categoria" maxlength="30" placeholder="Adicione uma categoria">
      <select name="categ" id="categ" class="span2" onchange="cat()">
        <option disabled="disabled" selected="selected"  value="" >Listar categorias</option>
		<?php $sql2 = mysql_query("SELECT * FROM categorias ");
				while($reg2 = mysql_fetch_array($sql2)){ ?>
        <option  value="<?php echo $reg2['categoria']; ?>"><?php echo $reg2['categoria']; ?></option>
		<?php } ?>        
      </select> </div>
  </div> 
...
<? if($op == "add_now"){ 
	...
	//testando campos da noticia
		if($titulo == "" || $texto == "" ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&op=add&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>"; die(); }
	...
	$sql= mysql_query("INSERT INTO $pagina (categoria,titulo, texto, hora, data, imagem, sessao, status )values('$categoria','$titulo','$texto','$hora','$data','$imagem','3','sim')") ;
	
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&op=add&msg=erro&txt=Ocorreu um erro ao inserir a notícia'>"; }
	
		else if($imagem == ""){  echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&msg=aviso&txt=<b>Imagem não enviada!</b> Notícia inserida com sucesso'>"; }
			
			else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&msg=sucesso&txt=Notícia inserida com sucesso'>"; }
...
<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=sessao_3&op=edit_now&id=<?= $id; ?>" onclick="return validaNoticia()">
...
<div  class="control-group ">
  ...
  <div id="campo1"  class="control-group ">
   ...
   <div id="campo2" class="control-group">
    <label class="control-label" for="fileInput">Texto da Notícia</label>
    <div class="controls" >
	    <textarea name="texto" style="width: 650px; height: 400px;" id="textarea"><?= stripslashes($reg['texto']); ?></textarea>
		<span id="erro2" class="help-inline"></span>
    </div>
  </div>
...
<? if($op == "edit_now"){ 
	...
	//testando campos da noticia
		if($titulo == "" || $texto == "" ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&op=edit&id=$id&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>";  die(); }	
	...
	$sql= mysql_query("UPDATE $pagina SET categoria='$categoria',titulo='$titulo',descricao='$descricao',texto='$texto',imagem='$imagem' WHERE id = '$id' ") ;
	
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&op=edit&id=$id&msg=erro&txt=Ocorreu um erro ao editar a notícia'>"; }
	
		else if($imagem == ""){  echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&msg=aviso&txt=<b>Imagem não enviada!</b> Notícia inserida com sucesso'>"; }
			
			else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&msg=sucesso&txt=Notícia editada com sucesso'>"; }
...
<ul class="breadcrumb bord" >
  ...
  <li>Excluir Notícia</li>
			...
			<p>
			<a class="btn btn-danger" href="?pag=sessao_3&op=exc&id=<?= $id;?>&msg=exc">Sim</a> <a class="btn" href="?pag=sessao_3">Não</a>
			</p>
			</div>
						<? }
		...
		if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&msg=erro&txt=Ocorreu um erro ao excluir a notícia'>"; }
	
		else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=sessao_3&msg=sucesso&txt=Notícia excluída com sucesso'>"; }
