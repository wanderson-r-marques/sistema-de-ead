...
<? }?>
	...
	?>
	  ...
	  <li><a href="?pag=juridico">Jurídico</a> <span class="divider">/</span></li>
      <li>Editar Jurídico</li>
      </ul>

	
<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=juridico&op=edit_now&id=<?= $id; ?>" >
...
<div class="control-group">
    <div class="controls">
      <button type="submit" class="btn" onclick="return validaEditjuridico()">Salvar</button>
    </div>
  </div>
</form>
...
<? if($op == "edit_now"){ 
	...
	//testando campos do prato  /*
	 if($texto == "" || $data == "" ){ echo "<meta http-equiv='refresh'  content='0;URL=?pag=juridico&op=edit&id=$id&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>";  die(); }				 
			
						
	$sql= mysql_query("UPDATE $pagina SET texto='$texto',data='$data' WHERE id = '$id' ") ;
	
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=juridico&op=edit&id=$id&msg=erro&txt=Ocorreu um erro ao editar '>"; }
	
	else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=juridico&msg=sucesso&txt=Jurídico editado com sucesso'>"; }
