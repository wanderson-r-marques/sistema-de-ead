...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered table-hover">
 <thead><tr>
	...
	<?php while($reg = mysql_fetch_array($sql)){ 
		 ...
		 $imagem = $reg['imagem'];
		 if($reg['imagem'] == "recados/user.png") $imagem =  '../recados/user.png'; 
	?>   
  <tr>
    <td>
   ...
   <td>
    ...
    <td>
		<a target="_blank" href="<?php echo sitei('siteurl');?>recados" class="btn" title="Ver recados" ><i class="icon-globe"></i></a>
		<a href="?pag=recados&op=sta&id=<?php echo $reg['id'];?>&status=<?php echo $status;?>" class="btn" title="<?php echo $title; ?>" ><i class="icon-eye-<?php echo $icon; ?>"></i></a>
		<a href="?pag=recados&op=exc&id=<?php echo $reg['id'];?>" class="btn" title="Excluir" ><i class="icon-trash"></i></a>	</td>
  </tr>
	<?php } ?>
...
<? if($op == "exc"){ 			
	...
	if($msg != "exc"){ ?>
			...
			<p>
			<a class="btn btn-danger" href="?pag=recados&op=exc&id=<?php echo $id;?>&msg=exc">Sim</a> <a class="btn" href="?pag=recados">Não</a>
			</p>
			</div>
						<? }
