...
<? if($op == "edit_now"){ 
	...
	//testando campos do prato  /*
	 if($texto == "" || $data == "" ){ echo "<meta http-equiv='refresh'  content='0;URL=?pag=radio&op=edit&id=$id&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>";  die(); }				 
			
						
	$sql= mysql_query("UPDATE $pagina SET texto='$texto',data='$data' WHERE id = '$id' ") ;
	
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=radio&op=edit&id=$id&msg=erro&txt=Ocorreu um erro ao editar '>"; }
	
	else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=radio&msg=sucesso&txt=Rádio editado com sucesso'>"; }
