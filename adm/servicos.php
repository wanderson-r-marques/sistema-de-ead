...
<? if($op == ""){ ?>
	...
	<!-- <a href="?pag=<?= $pagina; ?>&op=add" class="btn btn-primary" ><i class="icon-plus icon-white"></i> Adicionar novo menu </a><br />
<br /> -->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered table-hover">
 <thead><tr>
  ...
  <!-- <td><?php echo substr($reg['data'],8,2).'/'.substr($reg['data'],5,2).'/'.substr($reg['data'],0,4); ?><br /> -->
    ...
    <td>
		<a rel="tooltip" href="?pag=servicos&op=edit&id=<?= $reg['id'];?>" class="btn" title="Editar" ><i class="icon-pencil"></i></a>
		<a rel="tooltip" href="?pag=servicos&op=sta&id=<?= $reg['id'];?>&status=<?= $status;?>" class="btn" title="<?= $title; ?>" ><i class="icon-eye-<?= $icon; ?>"></i></a>
		<a rel="tooltip"  class="btn disabled" rel="tooltip"  title="Você não tem permissão"><i class="icon-trash"></i></a>	</td>
  </tr>
	<?php } ?>
...
?>
	  ...
	  <ul class="breadcrumb bord" >
      <li><a href="?">Home</a> <span class="divider">/</span></li>
	  <li><a href="?pag=servicos">Serviços</a> <span class="divider">/</span></li>
      <li>Adicionar Serviços</li>
      </ul>


<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=servicos&op=add_now">
  ...
  <div class="control-group">
    <div class="controls">
      <button type="submit" class="btn btn-primary"  onclick="return validaServiços()"><i class="icon-ok icon-white"></i> Salvar</button>
    </div>
  </div>
</form>
...
<? if($op == "add_now"){ 
	...
	$data = $_POST['data'];
	 ...
	 if($titulo == "" || $texto == "" ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&op=add&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>";  die(); }
	...
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&op=add&msg=erro&txt=Ocorreu um erro ao inserir o menu  '>"; }
	
		//else if($imagem == ""){  echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&msg=aviso&txt=<b>Imagem não enviada!</b> Menu inserido com sucesso'>"; }
			
			else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&msg=sucesso&txt=Menu inserido com sucesso'>"; }
...
<form name="formulario" class="form-horizontal form"  method="post" enctype="multipart/form-data" action="?pag=servicos&op=edit_now&id=<?= $id; ?>">
...
<div class="control-group ">
    ...
    <div class="controls" >
	      <input name="titulo"   readonly="" value="<?php echo $reg['titulo']; ?>" type="text" class="input-xlarge" id="titulo" maxlength="30" placeholder="Adicione um titulo com até 30 caracteres">
		  <span id="erro1" class="label label-important" style=" visibility:hidden;">* Insira uma título</span>
    </div>
  </div>    
  <div class="control-group ">
...
<? if($op == "edit_now"){ 
	...
	//testando campos do destaque  /*
	if($titulo == "" || $texto == ""  ){ echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&op=edit&id=$id&msg=erro&txt=<strong>Ocorreu um erro!</strong> Campos obrigatórios não foram preenchidos'>";  die(); }	
	...
	if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&op=edit&id=$id&msg=erro&txt=Ocorreu um erro ao editar o menu '>"; }
	
	//else if($imagem == ""){  echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&msg=aviso&txt=<b>Imagem não enviada!</b> Serviços editado com sucesso'>"; }
			
			else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&msg=sucesso&txt=Menu editado com sucesso'>"; }
...
<? if($op == "exc"){ 			
	...
	if($msg != "exc"){ ?>
			...
			<h4 class="alert-heading">Excluir Serviços</h4>
			<p>Tem certeza que deseja excluir este destaque? Esta operação não pode ser desfeita.</p>
			<p>
			<a class="btn btn-danger" href="?pag=servicos&op=exc&id=<?= $id;?>&msg=exc">Sim</a> <a class="btn" href="?pag=servicos">Não</a>
			</p>
			</div>
						<? }
		...
		if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&msg=erro&txt=Ocorreu um erro ao excluir o menu'>"; }
	
		else{  echo "<meta http-equiv='refresh' content='0;URL=?pag=servicos&msg=sucesso&txt=Menu excluído com sucesso'>"; }
...
<? if($op == "sta"){ 		
	...
	$sql= mysql_query("UPDATE $pagina SET status='$status' WHERE id = '$id' ") ;
		...
		if(!$sql){  echo "<meta http-equiv='refresh' content='0;URL= ?pag=$pagina&msg=erro&txt=Ocorreu um erro ao alterar o status do menu'>"; }
